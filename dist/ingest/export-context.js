 ...
 */
export function detectExportKind(relPaths) {
    const hasUsers = relPaths.includes('users.json');
    const hasChannels = relPaths.includes('channels.json');
    if (hasUsers && hasChannels && relPaths.some((p) => SLACK_DAY_FILE.test(p)))
        return 'slack';
    if (relPaths.some((p) => NOTION_FILE.test(p)))
        return 'notion';
    return null;
}
/**
 ...
 */
export function buildSlackUserMap(parsed) {
    const map = new Map();
...
//# sourceMappingURL=export-context.js.map