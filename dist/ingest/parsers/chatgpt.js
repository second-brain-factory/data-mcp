...
function canonicalPath(mapping, currentNode) {
    const path = [];
    ...
    while (cursor && !visited.has(cursor)) {
        ...
        if (!node)
            return []; // dangling pointer — caller skips the conversation
        path.push(node);
        cursor = node.parent;
    }
    return path.reverse();
...
export const parseChatGpt = (content) => {
    ...
    for (const convo of sorted) {
        if (!convo || typeof convo.mapping !== 'object' || !convo.mapping || typeof convo.current_node !== 'string')
            continue;
        const path = canonicalPath(convo.mapping, convo.current_node);
        if (path.length === 0)
            continue; // dangling current_node — skip, batch continues
        const messages = [];
        ...
        if (!messages.some((m) => m.text !== '[tool use]'))
            continue;
        conversations.push({
            title: convo.title ?? '',
            messages,
