...
export function renderSlackText(text, users) {
    return text
        .replace(/<@([A-Z0-9]+)(?:\|[^>]*)?>/g, (_m, id) => `@${users?.get(id) ?? id}`)
        .replace(/<#[A-Z0-9]+\|([^>]+)>/g, '#$1')
        .replace(/<(https?:\/\/[^|>]+)\|([^>]+)>/g, '$2 ($1)')
        .replace(/<(https?:\/\/[^>]+)>/g, '$1')
        .replace(/&lt;/g, '<')
...
export const parseSlackDay = (content, ctx) => {
    ...
    const keep = parsed.filter((m) => {
        if (m?.type !== 'message')
            return false;
        if (typeof m.subtype === 'string' && SKIP_SUBTYPES.has(m.subtype))
            return false;
    ...
    const name = (m) => (typeof m.user === 'string' ? users?.get(m.user) ?? m.user : 'unknown');
    const messages = [];
    const renderInto = (m, prefix) => {
        messages.push({ role: `${prefix}${name(m)}`, text: renderSlackText(m.text.trim(), users) });
        const ts = typeof m.ts === 'string' ? m.ts : '';
        for (const reply of replies.get(ts) ?? [])
            renderInto(reply, '  ↳ ');
    };
    ...
    for (const [threadTs, list] of replies) {
        if (!keep.some((m) => m.ts === threadTs && (!m.thread_ts || m.thread_ts === m.ts))) {
            for (const reply of list)
                messages.push({ role: name(reply), text: renderSlackText(reply.text.trim(), users) });
        }
    }
