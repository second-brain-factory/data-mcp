...
export function registerContactUpdate(server, adapter) {
    server.tool('contact_update', 'Update a contact record. Change any field by providing the new value.', {
        ...
        try {
            ...
            if (params.relationship !== undefined)
                updates.relationship = params.relationship;
            ...
            const record = await adapter.update('contacts', params.id, updates);
            return makeToolResponse({
                updated: true,
