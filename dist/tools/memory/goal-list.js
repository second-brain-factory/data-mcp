...
export function registerGoalList(server, adapter) {
    server.tool('goal_list', 'List goals with optional status and timeframe filters.', {
    ...
    }, { readOnlyHint: true }, withGracefulDegradation('goals', adapter, async (params) => {
        try {
            const clauses = [];
            if (params.status) {
            ...
            const result = await adapter.list('goals', {
                filter,
...
//# sourceMappingURL=goal-list.js.map