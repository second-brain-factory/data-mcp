...
export class PocketBaseAdapter implements DataAdapter {
    ...
    readonly backend: "pocketbase" = 'pocketbase';
    ...
    async create<T extends Record<string, unknown>>(collection: string, data: Record<string, unknown>): Promise<T> {
        ...
        try {
            ...
            return recordToPlain(record) as T;
        }
        catch (err) {
    ...
    async list<T extends Record<string, unknown>>(collection: string, options?: {
        ...
        try {
            ...
            const pbOptions: Record<string, string> = {};
            if (options?.filter) {
            ...
            return {
                items: result.items.map((r) => recordToPlain(r) as T),
    ...
    async textSearch<T extends Record<string, unknown>>(collection: string, query: string, options?: {
        ...
        try {
            ...
            return result.items.map((r) => recordToPlain(r) as T);
        }
        catch (err) {
            throw mapPocketBaseError(err, collection);
        }
    ...
    async upsert<T extends Record<string, unknown>>(collection: string, data: Record<string, unknown>, uniqueFields: string[]): Promise<T> {
        ...
        try {
            ...
            if (result.items.length > 0) {
                ...
                return recordToPlain(updated) as T;
            }
            // Create new
            const created = await this.pb.collection(collection).create(data);
            return recordToPlain(created) as T;
        }
        catch (err) {
            throw mapPocketBaseError(err, collection);
        }
 ...
 */
function recordToPlain(record: Record<string, unknown>): Record<string, unknown> {
...
function clauseToPocketBase(clause: FilterClause): string {
    ...
    validateFieldName(field);
    const formatValue = (v: FilterClause['value']): string => {
        if (v === null)
            return 'null';
        if (typeof v === 'string')
