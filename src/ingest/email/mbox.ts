...
import { parseEmailMessage, type ParsedEmail } from './mime.js';
...
export function unescapeFromLines(raw: string): string {
 ...
 * mime.ts.
 */
export async function readMbox(filePath: string): Promise<MboxResult> {
    ...
    const push = (l: string) => {
        if (currentChars >= MAX_RAW_MESSAGE_CHARS) return; // oversized message truncated
        current.push(l);
    ...
    for await (const chunk of stream) {
        ...
        for (const line of lines) {
            ...
            if (current.length === 0 && l.trim() === '') continue; // inter-message blank
            push(l);
        }
    }
    if (carry.length > 0) {
        const l = carry.endsWith('\r') ? carry.slice(0, -1) : carry;
        if (!ENVELOPE_LINE.test(l)) push(l);
        else flush();
    }
    flush();
