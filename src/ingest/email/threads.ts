 ...
 */
export function trimQuotedReplies(body: string): string {
    const lines = body.split('\n');
    const keep: string[] = [];
    let i = 0;
    const isQuoted = (l: string) => /^\s*>/.test(l);
    const isAttribution = (l: string) =>
        /^\s*(On .{0,200}wrote:?\s*$|-{2,}\s*(Original Message|Forwarded message)\s*-{2,})/i.test(l);
    while (i < lines.length) {
        if (isQuoted(lines[i])) {
            let j = i;
            while (j < lines.length && (isQuoted(lines[j]) || lines[j].trim() === '')) j++;
            const quotedCount = lines.slice(i, j).filter(isQuoted).length;
            if (quotedCount >= 2) {
                // drop the block, plus a preceding attribution line
                while (keep.length > 0 && (isAttribution(keep[keep.length - 1]) || keep[keep.length - 1].trim() === '')) keep.pop();
                i = j;
                continue;
            }
        }
...
function addresses(...headers: string[]): string[] {
    const out = new Set<string>();
    for (const h of headers) {
        for (const m of h.matchAll(/[\w.+-]+@[\w-]+\.[\w.-]+/g)) out.add(m[0].toLowerCase());
    }
    return [...out];
}
...
interface Thread {
    emails: ParsedEmail[];
    /** Index (into the input array) of each member, parallel to emails */
    indices: number[];
}
 ...
 */
export function groupEmailThreads(emails: ParsedEmail[], opts: ThreadGroupOptions = {}): ThreadGroupResult {
    const bulkIndices: number[] = [];
    const candidates: Array<{ email: ParsedEmail; index: number }> = [];
    ...
    for (let i = 0; i < emails.length; i++) {
        ...
        if (e.messageId && seenIds.has(e.messageId)) continue; // duplicate within batch
        if (e.messageId) seenIds.add(e.messageId);
        if (e.body.trim().length === 0 && e.subject.trim().length === 0) continue;
    ...
    const idToThread = new Map<string, Thread>();
    const subjectToThread = new Map<string, Thread>();
    const threads: Thread[] = [];

    for (const { email: e, index } of candidates) {
        const ids = [e.messageId, e.inReplyTo, ...e.references].filter((x): x is string => !!x);
        ...
        for (const id of ids) idToThread.set(id, thread);
        const subj = normalizeSubject(e.subject);
        if (subj && !subjectToThread.has(subj)) subjectToThread.set(subj, thread);
    }
    ...
    for (const thread of threads) {
        ...
        const dates = sorted.map((e) => e.date).filter((d): d is string => !!d);
