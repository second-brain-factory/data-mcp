...
export function stripNotionLinks(body: string): string {
    return body.replace(/\[([^\]]*)\]\(([^)]*)\)/g, (_m, text: string, target: string) => {
...
function folderTags(relPath: string | undefined): string[] {
    if (!relPath) return [];
...
export const parseNotionMd: Parser = (content: string, ctx: IngestContext): IngestItem[] => {
...
export const parseNotionDb: Parser = (content: string, ctx: IngestContext): IngestItem[] => {
