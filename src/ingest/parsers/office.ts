...
function sectionItems(markdown: string, docTitle: string): IngestItem[] {
...
function sheetItems(markdown: string, docTitle: string): IngestItem[] {
    const sections = splitSections(markdown);
    if (sections.length === 0) return [];
    const items: IngestItem[] = [];
    for (const section of sections) {
...
function slideItems(markdown: string, docTitle: string): IngestItem[] {
    const parts: Array<{ slide: number; text: string }> = [];
    let lastIndex = 0;
    ...
    if (lastSlide !== null) parts.push({ slide: lastSlide, text: markdown.slice(lastIndex).trim() });

    if (parts.length === 0) return sectionItems(markdown, docTitle);
...
export function parseOffice(markdown: string, ctx: OfficeContext): IngestItem[] {
    const docTitle = ctx.baseName;
    if (ctx.format === 'xlsx' || ctx.format === 'xls') return sheetItems(markdown, docTitle);
    if (ctx.format === 'pptx' || ctx.format === 'ppt') return slideItems(markdown, docTitle);
