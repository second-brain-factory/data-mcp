 ...
 */
export async function textSearchWithFallback<T extends Record<string, unknown>>(
    adapter: DataAdapter,
    collection: string,
    query: string,
    ...
    const primary = await adapter.textSearch<T>(collection, query, options);
    if (primary.length > 0) {
    ...
    const perTerm = await Promise.all(
        terms.map(async (term) => {
            try {
                return await adapter.textSearch<T>(collection, term, options);
            }
            catch {
                return [] as T[];
            }
    ...
    const byId = new Map<string, { item: T; matches: number; bestRank: number }>();
