...
export function registerBrainStats(server: McpServer, adapter: DataAdapter): void {
    server.tool('brain_stats', 'Get aggregate statistics about your Second Brain. Shows counts for all collections, knowledge breakdown by type, and stale item count.', {}, { readOnlyHint: true }, withGracefulDegradation('knowledge', adapter, async () => {
        try {
            ...
            const countEntries = await Promise.all(COLLECTIONS.map(async (collection): Promise<[string, number]> => {
                try {
            ...
            const typeEntries = await Promise.all(KNOWLEDGE_TYPES.map(async (type): Promise<[string, number]> => {
                try {
