...
const ENTITY_TYPES = ['knowledge', 'decision', 'session', 'blog_post', 'prospect', 'agent_learning'] as const;
...
export function registerLinkRelated(server: McpServer, adapter: DataAdapter): void {
    server.tool('link_related', 'Get all links for an entity. Shows outgoing and incoming relationships with resolved titles.', {
        ...
        try {
            ...
            if (params.direction === 'both' || params.direction === 'outgoing') {
                ...
                const result = await adapter.list<KnowledgeLink>('knowledge_links', {
                    filter: [filter],
                    sort: [{ field: 'created_at', direction: 'desc' }],
                    page: { limit: 50, offset: 0 },
                });
                for (const link of result.items) {
                    let targetTitle: unknown = null;
                    try {
