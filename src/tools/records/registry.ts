...
const nullable = (v: unknown): unknown => v ?? null;

export const RECORD_COLLECTIONS: Record<string, CollectionSpec> = {
    knowledge: {
    ...
    blog_posts: {
        ...
        buildCreate: (d) => {
            ...
            return {
                title: (d.title as string).trim(),
                slug: (d.slug as string) || slugify(d.title as string),
                content: d.content,
...
export const CREATABLE = Object.keys(RECORD_COLLECTIONS).filter((k) => RECORD_COLLECTIONS[k].createSchema) as [string, ...string[]];
export const UPDATABLE = Object.keys(RECORD_COLLECTIONS).filter((k) => RECORD_COLLECTIONS[k].updateSchema) as [string, ...string[]];
export const QUERYABLE = Object.keys(RECORD_COLLECTIONS).filter((k) => k !== 'knowledge_links') as [string, ...string[]];
export const DELETABLE = Object.keys(RECORD_COLLECTIONS).filter((k) => RECORD_COLLECTIONS[k].deletable) as [string, ...string[]];

/** Human-readable field spec for a zod object schema (for self-correcting errors). */
export function describeSchema(schema: z.ZodTypeAny): Record<string, string> {
    const out: Record<string, string> = {};
    const shape = (schema as z.ZodObject<z.ZodRawShape>).shape;
    for (const [key, value] of Object.entries(shape)) {
        let v = value as z.ZodTypeAny;
        let optional = false;
        while (v instanceof z.ZodOptional) {
            optional = true;
            v = v.unwrap() as z.ZodTypeAny;
        }
        let type: string;
        if (v instanceof z.ZodEnum)
            type = `enum: ${(v.options as string[]).join('|')}`;
