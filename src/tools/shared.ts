...
import type { AdapterErrorCode } from '../errors/adapter-error.js';
import type { DataAdapter } from '../adapter/types.js';
/** Build a successful MCP tool response */
export function makeToolResponse(data: unknown): {
    content: Array<{
        type: 'text';
        text: string;
    }>;
...
export function handleAdapterError(error: unknown, toolName: string): {
    content: Array<{
        type: 'text';
        text: string;
    }>;
 ...
 */
export function withGracefulDegradation<TParams>(collection: string, adapter: DataAdapter, handler: (params: TParams) => Promise<{
    content: Array<{
        type: 'text';
        text: string;
    }>;
